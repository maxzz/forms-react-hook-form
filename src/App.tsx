...
export function App() {
    const activePage = useSnapshot(appUi);
    return (
        <div className="min-h-screen grid grid-cols-[minmax(0,400px)_2rem_auto_2fr] bg-yellow-200">
            ...
            {activePage.activePage === ActivePage.page1 && <Page1_Form />}
            {activePage.activePage === ActivePage.page2 && <Page2_Form />}
            {activePage.activePage === ActivePage.page3 && <Page3_Form />}
        </div>
    );
}
